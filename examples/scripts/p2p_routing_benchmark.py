...
class Scenario:
    ...
    restart_count: int = 0
    shards: int = 1
    split_subscribers: bool = False
    # Combined-role workers (each process hosts pubs + subs + queryables
    # + getters). Used by the realistic-50 preset family. When workers > 0
    # the supervisor distributes 100+ topics + 100+ services across the
    # workers as described in z_p2p_routing_perf.rs Role::Worker.
    workers: int = 0
    n_pubs_per_worker: int = 2
    n_subs_per_worker: int = 2
    n_queryables_per_worker: int = 2
    n_getters_per_worker: int = 2
    get_period_ms: int = 1000
    get_timeout_ms: int = 2000
    # Publisher congestion control: "block" (legacy default; can hang
    # under Phase J ghost worker conditions) or "drop" (recommended
    # for mass-production where bounded latency matters more than
    # 100% delivery to a stuck destination).
    pub_congestion: str = "block"
    # Per-worker spawn stagger (ms). Default 0 spawns all workers
    # back-to-back. A small stagger (~50-100 ms) reliably prevents
    # the Phase J ghost-worker startup race; recommended for any
    # validation that targets mass-production reliability.
    worker_spawn_stagger_ms: int = 0
...
def main() -> int:
    ...
    for scenario_idx, scenario in enumerate(scenarios):
        for run_idx in range(runs):
            ...
            # Inter-scenario cleanup: peer-mode scenarios past the cliff
            # leave processes that supervisor never fully reaped (sessions
            # Block-waiting on dead pipelines). Accumulated across many
            # scenarios these orphans exhaust RLIMIT_NPROC for the user
            # on smaller hosts (observed on a 6-core ARM64 Linux device
            # at ~280 in-flight processes). Always cleanup + brief pause
            # so the next scenario starts from a clean slate.
            cleanup_orphans()
            time.sleep(2)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "shard-restart",
            "restart-blast-deep",
            "restart-blast-isolate",
            "restart-blast-isolate-n200",
            "restart-blast-isolate-n200-rc",
            "long-soak-1h-p2p",
            "long-soak-1h-p2p-n50",
            "long-soak-1h-router-discovery",
            "long-soak-1h-rc",
            "realistic-50-router-disc",
            "realistic-50-router-disc-drop",
            "realistic-50-production",
            "realistic-50-rc",
            "restart-blast-isolate-k5",
            "restart-blast-isolate-long",
        ],
        default="quick",
    )
...
def scenario_matrix(preset: str, include_200: bool) -> list[Scenario]:
    if preset == "quick":
        ...
        return scenarios
    elif preset == "restart-blast-isolate":
        # Phase 5.6b: separate mechanism 1 (dying peer's pipeline drain
        # blocks fan-out for topics the dying peer subscribes to) from
        # mechanism 2 (declare-storm + linkstate updates take the
        # wtable write lock and block route reads for *all* topics).
        # Setup: 2 topics, 2 publishers (one per topic), 20 subscribers
        # split disjoint (10 on topic 0, 10 on topic 1). At t=45s,
        # restart subscriber 0 (which belongs to topic 0's group).
        #
        # Expected analysis:
        #   - topic 0 subs (indices 2,4,6,8,10,12,14,16,18): see the
        #     full blast (mechanism 1 + mechanism 2)
        #   - topic 1 subs (indices 1,3,5,...,19): see mechanism 2 only
        #     (the restarted peer never subscribed to their topic)
        #   - delta between the two = magnitude of mechanism 1
        #   - topic 1 absolute = magnitude of mechanism 2
        scenarios = []
        for k in (0, 1, 3, 6):
            tag = "steady" if k == 0 else f"restart{k}"
            scenarios.append(
                Scenario(
                    f"n100_p2p_isolate_{tag}", "peer", "peer", 78, 20, 0,
                    publishers=2, topics=2,
                    split_subscribers=True,
                    restart_at_secs=0 if k == 0 else 45,
                    restart_count=k,
                )
            )
        return scenarios
    elif preset == "restart-blast-isolate-n200":
        # Phase 8 P0: same isolate methodology at N=200. Same
        # subscriber count (20 split 10/10), more idle peers
        # (178). Confirms that Plan A + P7.1 gains hold up at
        # 2× mesh size. Memory budget ≈ 2 GB tree RSS at N=200.
        scenarios = []
        for k in (0, 1, 3, 6):
            tag = "steady" if k == 0 else f"restart{k}"
            scenarios.append(
                Scenario(
                    f"n200_p2p_isolate_{tag}", "peer", "peer", 178, 20, 0,
                    publishers=2, topics=2,
                    split_subscribers=True,
                    restart_at_secs=0 if k == 0 else 45,
                    restart_count=k,
        ...
        return scenarios
    elif preset == "realistic-50-router-disc":
        # Goal scenario: 50 peer processes (workers) + 1 router for
        # discovery. Each worker hosts 2 pubs + 2 subs + 2 queryables +
        # 2 getters → system carries 100 topics + 100 services
        # uniformly distributed across all 50 peers. 3 of the peers are
        # additionally cycled (open/declare/close) to stress the
        # close-path under realistic distributed load. This is the
        # primary test for the issue-2581 large-mesh-with-churn goal.
        #
        # Roles balance:
        #   - 50 workers (all peers, connected to router for discovery,
        #     p2p direct for data); each hosts the full mix
        #   - 3 churners (separate processes, peer mode) doing
        #     continuous restart cycles to inject hostile load
        #   - 1 hub (router mode) for gossip only
        # Total processes: 50 workers + 3 churners + 1 router = 54.
        scenarios = [
            Scenario(
                "realistic_50_router_disc",
                "router", "peer",
                idle_peers=0,
                publishers=0,
                subscribers=0,
                ...
                n_pubs_per_worker=2,
                n_subs_per_worker=2,
                n_queryables_per_worker=2,
                n_getters_per_worker=2,
                get_period_ms=1000,
                get_timeout_ms=2000,
                split_subscribers=False,
                restart_at_secs=0,
                restart_count=0,
            ),
        ]
        return scenarios
    elif preset == "realistic-50-router-disc-drop":
        # Same as realistic-50-router-disc but with publisher
        # congestion_control=Drop. Recommended for mass-production
        # validation: prevents Phase J ghost workers from hanging
        # publishers indefinitely; lets the system continue serving
        # other destinations when one peer is in a bad state.
        scenarios = [
            Scenario(
                "realistic_50_router_disc_drop",
                "router", "peer",
                idle_peers=0,
                publishers=0,
                subscribers=0,
        ...
        return scenarios
    elif preset == "realistic-50-production":
        # **The production-grade preset**: Drop-congestion publishers
        # plus 100 ms inter-worker spawn stagger. The stagger maps to
        # a realistic robot-fleet boot pattern (devices come online
        # over a 5-second window, not simultaneously) and eliminates
        # the Phase J cold-start ghost-worker race observed under
        # 50 simultaneous opens. Pair with the standard 3 continuous
        # churners.
        scenarios = [
            Scenario(
                "realistic_50_production",
                "router", "peer",
                idle_peers=0,
                publishers=0,
                subscribers=0,
        ...
        return scenarios
    elif preset == "realistic-50-rc":
        # Same realistic distribution but with router-and-clients
        # topology: hub is router and leaves (workers) are clients, so
        # all data flows through the router. Lets us compare service
        # quality (query latency / timeout rate / pub-sub loss) when
        # routing is centralized vs. peer-direct.
        scenarios = [
            Scenario(
                "realistic_50_rc",
                "router", "client",
        ...
        return scenarios
    elif preset == "long-soak-1h-router-discovery":
        # Phase H router-discovery variant: 1 router hub + 50 peer
        # leaves connecting to it for gossip/discovery. Peers form
        # direct p2p connections to each other for data. The router
        # itself does not route data — it just runs the gossip
        # backbone. Goal: measure router-side RSS/CPU under continuous
        # peer churn, plus end-to-end message loss / latency.
        scenarios = [
            Scenario(
                "n50_router_disc_soak1h",
                "router", "peer",  # hub is router; leaves are peers
                36, 10, 3,
        ...
        return scenarios
    elif preset == "long-soak-1h-p2p-n50":
        # Phase H N=50 variant: same 1-hour soak shape but N=50 to
        # observe whether high sys-time CPU is really O(N^2) mesh
        # overhead. Layout: 36 idle + 1 pub + 10 subs + 3 churners
        # = 50 leaves + 1 hub.
        scenarios = [
            Scenario(
                "n50_p2p_soak1h",
        ...
        return scenarios
    elif preset in ("long-soak-1h-p2p", "long-soak-1h-rc"):
        # Phase H: 1-hour soak with 3 continuous churners at N=100.
        # Goal: verify (a) memory does not grow, (b) no per-peer CPU
        # spike, (c) no put/get stalls under continuous restart load.
        #
        # Layout: 86 idle + 1 publisher + 10 subscribers + 3 churners
        # = 100 total leaves. Churners do open/declare/undeclare/close
        # cycles continuously at the default 20 ms hold + 20 ms idle
        # (≈ 25 cycles/sec/churner ≈ 75 cycles/sec aggregate ≈ 270 k
        # cycles per hour).
        hub_mode = "peer" if preset.endswith("p2p") else "router"
        leaf_mode = "peer" if preset.endswith("p2p") else "client"
        tag = "p2p" if preset.endswith("p2p") else "rc"
        scenarios = [
            Scenario(
                f"n100_{tag}_soak1h",
                hub_mode,
        ...
        return scenarios
    elif preset == "restart-blast-isolate-n200-rc":
        # Goal validation P0 (E3 reframe): same isolate methodology at
        # N=200 but in router/client mode so all 200 nodes fit in the
        # 5.8 GiB device's RAM budget (peer mode at N=200 OOM-kills
        # ~50% in: each peer process ~30 MB × 200 = ~6 GB tree RSS).
        # Confirms whether surviving clients keep low rx tail when
        # 1/3/6 of them are restarted mid-stream.
        scenarios = []
        for k in (0, 1, 3, 6):
            tag = "steady" if k == 0 else f"restart{k}"
            scenarios.append(
                Scenario(
                    f"n200_rc_isolate_{tag}", "router", "client", 178, 20, 0,
                    publishers=2, topics=2,
                    split_subscribers=True,
                    restart_at_secs=0 if k == 0 else 45,
                    restart_count=k,
        ...
        return scenarios
    elif preset == "restart-blast-isolate-k5":
        # Phase 8 P0: same isolate methodology with K=5 (5 topics,
        # 5 publishers, 20 subs split 4/4/4/4/4 across topics).
        # idle = 100 - 5 - 20 - 1 = 74. Tests whether the fix
        # holds up when each repropagate iteration must process 5×
        # the declares.
        scenarios = []
        for k in (0, 1, 3, 6):
            tag = "steady" if k == 0 else f"restart{k}"
            scenarios.append(
                Scenario(
                    f"n100_p2p_isolate_k5_{tag}", "peer", "peer", 74, 20, 0,
                    publishers=5, topics=5,
                    split_subscribers=True,
                    restart_at_secs=0 if k == 0 else 45,
                    restart_count=k,
        ...
        return scenarios
    elif preset == "restart-blast-isolate-long":
        # Phase 8 P1: long-duration soak. 5-min runs with restart
        # event at t=150 (mid-stream, well past startup ramp).
        # Confirms no contention accumulation, memory leak, or
        # tail-latency drift over a longer window.
        scenarios = [
            Scenario(
                "n100_p2p_isolate_long_steady", "peer", "peer", 78, 20, 0,
                publishers=2, topics=2,
                split_subscribers=True,
                restart_at_secs=0,
                restart_count=0,
            ...
            Scenario(
                "n100_p2p_isolate_long_restart3", "peer", "peer", 78, 20, 0,
                publishers=2, topics=2,
                split_subscribers=True,
                restart_at_secs=150,
                restart_count=3,
        ...
        return scenarios
    elif preset == "restart-blast-deep":
        # Phase 5.6: characterize peer-mode restart impact on *already-
        # running* peers, after steady state is fully established.
        # The existing restart-sweep preset restarts at t=15s into a
        # 30s run — that's too close to startup to distinguish startup
        # cliff effects from restart blast. This preset uses a 90s run
        # with the restart event at t=45s, giving:
        #   - 0–45s startup + steady-state ramp (compare with steady=0)
        #   - 45s: kill restart_count subscribers
        #   - 45–90s: 45s of post-restart observation, plenty to see
        #             whether the spike is short or persistent
        # peer mode only — this is the regime where the supposed
        # restart blast radius problem lives. Pair with the steady
        # control (restart_count=0) at the same N to subtract noise.
        scenarios = []
        for k in (0, 1, 3, 6):
            tag = "steady" if k == 0 else f"restart{k}"
            scenarios.append(
                Scenario(
                    f"n100_p2p_deep_{tag}", "peer", "peer", 79, 20, 0,
                    publishers=1, topics=1,
                    restart_at_secs=0 if k == 0 else 45,
                    restart_count=k,
                )
        ...
        return scenarios
    elif preset == "linux-cliff":
        # Phase 3.5: find Linux ARM64's actual router-hybrid cliff.
        # On Linux the N=150 single-router stress that we saw on macOS
        # does not reproduce, so the cliff (if any) is further out. We
        # push N to {150, 200, 300, 400, 500} and compare shards={1,2}
        # to see (a) whether a router-hybrid single-router stress point
        # exists at all on Linux RT in this range, and (b) at what N
        # shards=2 becomes a measurable win.
        #
        # Peer mode is intentionally absent — peer mode at N≥125 leaks
        # processes that exhaust the cgroup pids limit across many
        # scenarios. Peer mode behavior was already characterised in
        # the regular shard-sweep preset; we don't need to re-prove it
        # at higher N on the device.
        scenarios = []
        for n in (150, 200, 300, 400, 500):
            idle = n - 1 - 20
            if idle < 0:
                continue
            for s in (1, 2):
                scenarios.append(
                    Scenario(
                        f"n{n:03d}_s{s}_cli", "router", "client",
                        idle, 20, 0, publishers=1, topics=1, shards=s,
                    )
                )
        return scenarios
    elif preset == "shard-restart":
        # Phase 3.5: does the shard backbone amplify or contain
        # restart blast radius? Sweep shards over {1, 2, 3} at the
        # N=150 single-router stress point (macOS-specific stress
        # but the test is interesting on both hosts: does the
        # restart_count=6 event recover quickly with more shards?).
        # restart_at_secs=15 into a 30 s run, restart_count=6
        # (30% of 20 subscribers).
        scenarios = []
        for s in (1, 2, 3):
            scenarios.append(
                Scenario(
                    f"n150_s{s}_cli_restart6", "router", "client",
                    129, 20, 0, publishers=1, topics=1, shards=s,
                    restart_at_secs=15, restart_count=6,
                )
            )
            scenarios.append(
                Scenario(
                    f"n150_s{s}_cli_steady", "router", "client",
                    129, 20, 0, publishers=1, topics=1, shards=s,
                )
            )
        return scenarios
    elif preset == "shard-sweep":
        # Phase 3 / Track B prototype: does splitting the mesh into M
        # independent shards (each with its own hub) move the cliff?
        # Fixed workload: 1 publisher + 20 subscribers + idle filler.
        # Idle filler shifts to push total session count to N.
        # Sweep N over {100, 150, 200} and shards over {1, 2, 3, 4}.
        # We focus on router/client mode because peer-mode gossip would
        # propagate state across shards anyway (defeating the purpose);
        # router-hybrid is the cleanest "sharding" available without
        # changing zenoh internals. We also include peer/peer at
        # shards=1 as a control to confirm the baseline cliff has not
        # shifted from prior runs.
        scenarios = []
        for n in (100, 150, 200):
            ...
            # peer/peer control at shards=1 only — shards>1 with peer
            # gossip doesn't actually isolate state (linkstate floods
            # across bridges), so the comparison wouldn't be apples-to-
            # apples. The control row confirms cliff baseline.
            scenarios.append(
                Scenario(
                    f"n{n:03d}_s1_p2p", "peer", "peer",
                    idle, 20, 0, publishers=1, topics=1, shards=1,
                )
            )
        return scenarios
    elif preset == "n-sweep":
...
def supervisor_command(
    ...
    command = [
        ...
        str(scenario.shards),
        "--split-subscribers",
        str(scenario.split_subscribers).lower(),
        "--payload-size",
        str(payload_size),
        "--put-period-ms",
        str(put_period_ms),
        ...
        "--n-pubs-per-worker",
        str(scenario.n_pubs_per_worker),
        "--n-subs-per-worker",
        str(scenario.n_subs_per_worker),
        "--n-queryables-per-worker",
        str(scenario.n_queryables_per_worker),
        "--n-getters-per-worker",
        str(scenario.n_getters_per_worker),
        "--get-period-ms",
        str(scenario.get_period_ms),
        "--get-timeout-ms",
        str(scenario.get_timeout_ms),
        "--pub-congestion",
        scenario.pub_congestion,
        "--worker-spawn-stagger-ms",
        str(scenario.worker_spawn_stagger_ms),
    ]
    for item in cfg:
        command += ["--cfg", item]
    return command
...
def _sample_loop(
    ...
    fields = [
        ...
        "tree_max_rss_kb",
        "tree_total_cpu_pct",
        # Phase G additions for per-peer spike detection in long soaks:
        "tree_max_cpu_pct",
        "tree_p99_rss_kb",
        "tree_p99_cpu_pct",
    ]
            ...
            writer.writerow(
                {
                    ...
                    "tree_max_cpu_pct": f"{sample.get('max_cpu_pct', 0.0):.2f}",
                    "tree_p99_rss_kb": sample.get("p99_rss_kb", 0),
                    "tree_p99_cpu_pct": f"{sample.get('p99_cpu_pct', 0.0):.2f}",
                }
            )
            f.flush()
...
def sample_proc_tree(supervisor_pid: int) -> dict[str, Any] | None:
    ...
    def _percentile(values: list[float], p: float) -> float:
        ...
        k = (len(ordered) - 1) * p
        f_idx = int(k)
        c_idx = min(f_idx + 1, len(ordered) - 1)
        return ordered[f_idx] + (ordered[c_idx] - ordered[f_idx]) * (k - f_idx)
    ...
    return {
        ...
        "max_cpu_pct": max(cpu_values, default=0.0),
        "p99_rss_kb": int(_percentile([float(v) for v in rss_values], 0.99)),
        "p99_cpu_pct": _percentile(cpu_values, 0.99),
    }
