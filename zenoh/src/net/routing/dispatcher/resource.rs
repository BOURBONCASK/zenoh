...
impl Resource {
    ...
    /// Pure-read variant of [`Resource::decl_key`] that returns a full-path
    /// `WireExpr` and **never** mutates the resource tree or the face's
    /// mapping tables. Used by Phase 5 (PR 3) on the
    /// `new_transport_unicast_face` rejoin path: the original
    /// `decl_key_deferred` mutates `nonwild_prefix.face_ctxs` and
    /// `face.local_mappings` inside the `wtables` write critical section
    /// for every propagated resource. When a peer restart triggers a
    /// rejoin storm, these mutations dominate the cumulative wtable hold
    /// (see `phase5_report.md`).
    ///
    /// Tradeoff: callers using this variant skip the compact-keyexpr
    /// optimization for the *first* declare batch to a new face. Future
    /// declares to the same face will still register and reuse compact
    /// ids through the regular `decl_key_deferred` path. The wire-size
    /// cost is one declare per resource carrying the full path instead
    /// of `scope=expr_id, suffix=<remainder>`; for typical sub paths
    /// (< 80 bytes) the cost is negligible relative to the saved
    /// critical-section time.
    #[inline]
    pub(crate) fn decl_key_simple(res: &Arc<Resource>) -> WireExpr<'static> {
