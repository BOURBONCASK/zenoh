...
use zenoh_protocol::{
    core::{EndPoint, Field, Locator, Priority, RegionName, Resolution, WhatAmI, ZenohIdProto},
    transport::BatchSize,
    VERSION,
};
...
pub type BoundCallback = Arc<dyn Fn(TransportPeer) -> ZResult<Bound> + Send + Sync>;
...
impl TransportManagerBuilder {
    ...
    pub fn bound_callback(
        mut self,
        callback: impl Fn(TransportPeer) -> ZResult<Bound> + Send + Sync + 'static,
    ) -> Self {
    ...
    pub async fn from_config(
        ...
        self = self.region_name(config.gateway.region_name.clone());
